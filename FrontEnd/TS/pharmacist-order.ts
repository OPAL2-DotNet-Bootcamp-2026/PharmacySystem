(() => {
  ...
  function getUserIdFromToken(): number | null {
    ...
    try {
      ...
      const parsed: unknown = JSON.parse(atob(paddedBase64));
  ...
  async function initialise(): Promise<void> {
    ...
    const pharmacySelect = requireElement<HTMLSelectElement>("#pharmacyId");
    const medicineSelect = requireElement<HTMLSelectElement>("#medicineId");
    const quantityInput = requireElement<HTMLInputElement>("#quantity");
    const addMedicineButton =
      requireElement<HTMLButtonElement>(".btn-add-medicine");
    const submitButton =
      requireElement<HTMLButtonElement>(".btn-submit-order");
    const orderLines = requireElement<HTMLDivElement>("#orderLines");
    const estimatedTotal = requireElement<HTMLElement>("#estimatedTotal");
    const allOrdersTableBody =
      requireElement<HTMLTableSectionElement>("#allOrdersTableBody");
    const myOrdersTableBody =
      requireElement<HTMLTableSectionElement>("#myOrdersTableBody");
    ...
    async function loadPharmacies(): Promise<void> {
      ...
      const options = pharmacies
        ...
        .map(
          (pharmacy) =>
            new Option(pharmacy.pharmacyName, String(pharmacy.pharmacyID)),
        );
    ...
    async function loadMedicines(): Promise<void> {
      medicines = await Api.get<Medicine[]>("/Medicine/GetAvailable");
      const placeholder = new Option("Select medicine", "");
      const options = medicines.map(
        (medicine) =>
          new Option(medicine.medicineName, String(medicine.medicineID)),
      );

      medicineSelect.replaceChildren(placeholder, ...options);
    }

    async function loadCurrentPharmacist(): Promise<void> {
      ...
      const pharmacists = await Api.get<Pharmacist[]>("/Pharmacist");
      currentPharmacist =
        pharmacists.find(
          (pharmacist) => pharmacist.userID === userId && pharmacist.isActive,
        ) ?? null;
    ...
    function renderOrderDetails(): void {
      ...
      orderDetails.forEach((detail, index) => {
        ...
        removeButton.setAttribute(
          "aria-label",
          `Remove ${detail.medicineName} from the order`,
        );
    ...
    function addMedicine(): void {
      ...
      const existingMedicine = orderDetails.find(
    ...
    function renderAllOrders(orders: PharmacistOrder[]): void {
    ...
    function renderMyOrders(orders: PharmacistOrder[]): void {
    ...
    function updateOrderCounts(orders: PharmacistOrder[]): void {
      const pendingCount = orders.filter((order) => order.status === "Pending").length;

      document.querySelectorAll<HTMLElement>("[data-order-count]").forEach((badge) => {
    ...
    async function loadOrders(): Promise<void> {
      try {
        const orders = await Api.get<PharmacistOrder[]>("/PharmacistOrder");
        ...
        if (pharmacist) {
          renderMyOrders(
            orders.filter(
              (order) => order.pharmacistID === pharmacist.pharmacistID,
            ),
          );
        }
      } catch (error: unknown) {
    ...
    async function createPharmacistOrder(): Promise<void> {
      ...
      try {
        await Api.post<CreateOrderResponse, CreateOrderRequest>(
          "/PharmacistOrder",
          order,
        );
    ...
    async function updateOrderStatus(
      ...
      try {
        await Api.put<string, UpdateOrderStatusRequest>(
    ...
    if (role === "Pharmacist") {
      ...
      orderLines.addEventListener("click", (event: MouseEvent) => {
    ...
    if (role === "Admin" || role === "Manager") {
      allOrdersTableBody.addEventListener("click", (event: MouseEvent) => {
        if (!(event.target instanceof Element)) {
          return;
        }

        const approveButton =
          event.target.closest<HTMLButtonElement>("[data-approve-id]");
        const rejectButton =
          event.target.closest<HTMLButtonElement>("[data-reject-id]");
        ...
        const orderID = Number(
          approveButton?.dataset.approveId ?? rejectButton?.dataset.rejectId,
        );
