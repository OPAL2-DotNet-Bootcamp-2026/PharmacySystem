...
document.addEventListener("DOMContentLoaded", async (): Promise<void> => {
    ...
    const footer = document.querySelector<HTMLElement>(".small-footer");
    ...
    try {
        ...
        for (const pharmacy of pharmacies) {
            const pharmacyID: number = pharmacy.pharmacyID;

            let stocks: PharmacyStock[] = [];

            try {
                stocks = await Api.get<PharmacyStock[]>(
    ...
    function createStockRows(stocks: PharmacyStock[]): string {
