...
(() => {
    ...
    async function initialise() {
        ...
        async function loadMedicines() {
            medicines =
                await Api.get("/Medicine/GetAvailable");
        ...
        async function loadCurrentPharmacist() {
            ...
            if (!userId) {
                throw new Error("Your user ID could not be read " +
                    "from the login token.");
            }
            const pharmacists = await Api.get("/Pharmacist");
            currentPharmacist =
                pharmacists.find((pharmacist) => pharmacist.userID === userId &&
                    pharmacist.isActive) ?? null;
        ...
        function renderOrderDetails() {
            ...
            orderDetails.forEach((detail, index) => {
                ...
                removeButton.setAttribute("aria-label", `Remove ${detail.medicineName} ` +
                    "from the order");
        ...
        function updateOrderCounts(orders) {
            ...
            document
                .querySelectorAll("[data-order-count]")
                .forEach((badge) => {
                badge.textContent =
                    String(pendingCount);
            });
        ...
        async function loadOrders() {
            try {
                ...
                if (pharmacist) {
                    renderMyOrders(orders.filter((order) => order.pharmacistID ===
                        pharmacist.pharmacistID));
                }
            }
            catch (error) {
                console.error("Failed to load orders:", error);
                const target = role === "Pharmacist"
                    ? myOrdersTableBody
                    : allOrdersTableBody;
        ...
        async function createPharmacistOrder() {
            if (role !== "Pharmacist" ||
                !currentPharmacist) {
                alert("An active pharmacist profile " +
        ...
        if (role === "Admin" ||
            ...
            allOrdersTableBody.addEventListener("click", (event) => {
                ...
                const orderID = Number(approveButton?.dataset.approveId ??
                    rejectButton?.dataset.rejectId);
                ...
                void updateOrderStatus(orderID, approveButton
                    ? "Approved"
                    : "Cancelled");
            });
        }
